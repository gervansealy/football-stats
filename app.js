class FootballDatabase {
    ...
    setupEventListeners() {
        document.querySelectorAll('.nav-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (e.target.dataset.section === 'input') {
                    if (this.userPermission === 'viewer') {
        ...
        document.querySelectorAll('.tab-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (e.target.dataset.tab === 'game-history') {
                    ...
                    this.renderGameHistory();
                } else if (e.target.dataset.tab === 'input-stats') {
    ...
    addPlayer() {
        ...
        const headshotLink = document.getElementById('headshot').value.trim();
        ...
        if (editingId) {
            ...
            if (player) {
                ...
                if (headshotLink) {
                    player.headshot = this.formatImageUrl(headshotLink);
                } else {
            ...
            const player = {
                ...
                birthday,
                hobbies,
                headshot: headshotLink ? this.formatImageUrl(headshotLink) : null,
                highlights: [],
                stats: {
                    gamesPlayed: 0,
                    wins: 0,
    ...
    editPlayer(playerId) {
        ...
        document.getElementById('headshot').value = player.headshot || '';
    ...
    renderBulkStatsForm() {
        ...
        newContainer.innerHTML = this.players.map(player => `
            <div class="player-stat-card">
                ...
                <div class="stat-inputs">
                    ...
                    <div class="form-group">
                        <label>
                            <input type="checkbox" class="stat-captain-draw" data-player="${player.id}">
                            Captain Draw
                        </label>
                    </div>
                    <div class="form-group">
        ...
        newContainer.addEventListener('change', (e) => {
            ...
            if (target.classList.contains('stat-win') || 
                target.classList.contains('stat-draw') || 
                target.classList.contains('stat-loss')) {
                
                const playerId = target.dataset.player;
                const winCheckbox = newContainer.querySelector(`.stat-win[data-player="${playerId}"]`);
                const drawCheckbox = newContainer.querySelector(`.stat-draw[data-player="${playerId}"]`);
                const lossCheckbox = newContainer.querySelector(`.stat-loss[data-player="${playerId}"]`);
                
                if (target.checked) {
                    if (target.classList.contains('stat-win')) {
                        if (drawCheckbox) drawCheckbox.checked = false;
                        if (lossCheckbox) lossCheckbox.checked = false;
                    } else if (target.classList.contains('stat-draw')) {
                        if (winCheckbox) winCheckbox.checked = false;
                        if (lossCheckbox) lossCheckbox.checked = false;
                    } else if (target.classList.contains('stat-loss')) {
                        if (winCheckbox) winCheckbox.checked = false;
                        if (drawCheckbox) drawCheckbox.checked = false;
                    }
                }
    ...
    submitAllStats() {
        ...
        this.players.forEach(player => {
            ...
            const captainDraw = container.querySelector(`.stat-captain-draw[data-player="${player.id}"]`).checked;
            const captainLoss = container.querySelector(`.stat-captain-loss[data-player="${player.id}"]`).checked;

            if (win || draw || loss || cleanSheet || goals > 0 || captainWin || captainDraw || captainLoss) {
    ...
    populateYearFilters() {
        ...
        [standingsFilter, inputFilter].forEach(filter => {
    ...
    calculateStats(player) {
        ...
        const points = 
            ...
            (stats.captainDraws * this.pointValues.captainDraw) +
    ...
    updateStandings() {
        ...
        sortedPlayers.forEach((player, index) => {
            ...
            row.innerHTML = `
                <td>${index + 1}</td>
                <td><span class="player-name-link" data-player-id="${player.id}">${player.firstName} ${player.lastName}</span></td>
                ...
                <td>${player.stats.captainDraws}</td>
            ...
            const nameLink = row.querySelector('.player-name-link');
        ...
        if (this.players.length === 0) {
            tbody.innerHTML = '<tr><td colspan="14" class="empty-state">No players added yet</td></tr>';
        }
    }
    ...
    displayPlayerProfile(playerId) {
        ...
        profileView.innerHTML = `
            ...
            <div class="profile-main-grid">
                ...
                <div class="profile-right">
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="stat-item">
                            <span class="stat-label">Captain Draws</span>
                            <span class="stat-value">${player.stats.captainDraws}</span>
                        </div>
                        <div class="stat-item">
            ...
            <div class="profile-bottom-sections">
                ...
                <div class="profile-section">
                    ...
                    ${this.isEditor ? `
                        <div class="video-upload-section">
                            <input type="text" id="video-url-${playerId}" placeholder="Paste YouTube, Drive, Dropbox, or direct video link" style="flex: 1; padding: 8px; border: 1px solid #ddd; border-radius: 4px; font-size: 0.85rem;">
                            <input type="text" id="video-title-${playerId}" placeholder="Video title (optional)" style="flex: 0.5; padding: 8px; border: 1px solid #ddd; border-radius: 4px; margin-left: 8px; font-size: 0.85rem;">
                            <button class="btn-secondary" onclick="database.addVideoLink('${playerId}')" style="margin-left: 8px; padding: 8px 12px; font-size: 0.85rem;">➕ Add</button>
                        </div>
                    ` : ''}
    ...
    formatImageUrl(url) {
        // Google Drive detection - convert to direct image URL
    ...
    async checkDataStatus() {
        alert(`Current Data Status:
        
Players in memory: ${this.players.length}
Games in memory: ${this.games.length}

Checking Firebase...`);

        try {
            const playersFromDB = await this.loadData('players');
            const gamesFromDB = await this.loadData('games');
            
            alert(`Firebase Data:
            
Players in Firebase: ${playersFromDB ? playersFromDB.length : 0}
Games in Firebase: ${gamesFromDB ? gamesFromDB.length : 0}

${playersFromDB && playersFromDB.length > 0 ? 'YOUR PLAYERS ARE STILL IN FIREBASE! Click "Restore from Firebase" to get them back.' : 'No players found in Firebase.'}`);
            
            console.log('Players from Firebase:', playersFromDB);
            console.log('Games from Firebase:', gamesFromDB);
        } catch (error) {
    ...
    async restoreFromFirebase() {
        if (!confirm('This will reload all data from Firebase. Continue?')) {
            return;
        }
...
Games: ${this.games.length}`);
            ...
            alert('Error restoring from Firebase: ' + error.message);
            console.error(error);
        }
    }
    ...
    loadSettings() {
        ...
        document.getElementById('points-captain-draw').value = this.pointValues.captainDraw;
    ...
    saveSettings() {
        this.pointValues = {
            ...
            captainDraw: parseFloat(document.getElementById('points-captain-draw').value),
    ...
    editGame(gameId) {
        ...
        setTimeout(() => {
            ...
            game.playerStats.forEach(stat => {
                ...
                const captainDrawCheckbox = container.querySelector(`.stat-captain-draw[data-player="${stat.playerId}"]`);
                ...
                if (captainDrawCheckbox) captainDrawCheckbox.checked = stat.captainDraw;
    ...
    verifyPassword() {
        ...
        if (!this.accessPasswords[role]) {
            alert('Password not configured for this role. Please contact admin.');
            return;
        }
    ...
    openAdminSettings() {
        ...
        const viewerPass = document.getElementById('viewer-password');
        
        adminPass.value = this.accessPasswords.admin || '';
        editorPass.value = this.accessPasswords.editor || '';
        viewerPass.value = this.accessPasswords.viewer || '';
    ...
    saveAccessPasswords() {
        ...
        const viewerPass = document.getElementById('viewer-password').value.trim();
        ...
        if (!viewerPass) {
            alert('Viewer password is required!');
            return;
        }
