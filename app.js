...
function calculatePlayerStats(playerId) {
    ...
    for (const gameId in gamesData) {
        ...
        if (game.playerStats && game.playerStats[playerId]) {
            ...
            if (playerGame.result === 'win') {
                stats.wins++;
                if (playerGame.wasCaptain) stats.captainWins++;
            }
            else if (playerGame.result === 'draw') {
                stats.draws++;
                if (playerGame.wasCaptain) stats.captainDraws++;
            }
            else if (playerGame.result === 'loss') {
                stats.losses++;
                if (playerGame.wasCaptain) stats.captainLosses++;
            }
...
function loadSettingsForm() {
    ...
    document.getElementById('adminPassword').value = settingsData.adminPassword || 'admin123';
    document.getElementById('viewerPassword').value = settingsData.viewerPassword || 'viewer123';
}
...
function savePasswords() {
    ...
    const viewerPassword = document.getElementById('viewerPassword').value;
    ...
    if (adminPassword === viewerPassword) {
        alert('Admin and Viewer passwords must be different');
        return;
    }
    ...
    database.ref('settings').update({
        ...
        alert('Error updating passwords: ' + error.message);
    });
}
...
function playVideo(videoUrl, playerName, videoNumber) {
    ...
    if (videoUrl.includes('youtube.com') || videoUrl.includes('youtu.be')) {
        ...
        if (videoId) {
            ...
            videoPlayer.nextElementSibling.innerHTML = `<iframe width="100%" height="480" src="https://www.youtube.com/embed/${videoId}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
        }
