class FootballDatabase {
    constructor() {
        ...
        this.pointValues = this.loadData('pointValues') || {
            win: 3,
            draw: 1,
            loss: 0,
            cleanSheet: 1,
        ...
        this.previousYearTitles = this.loadData('previousYearTitles') || {};
        this.userPermission = this.loadData('userPermission') || null;
        this.isEditor = this.userPermission === 'editor' || this.userPermission === 'admin';
        this.accessPasswords = this.loadData('accessPasswords') || {
            admin: '',
            editor: '',
            viewer: ''
        };
            ...
            <div class="profile-section">
                ...
                ${this.isEditor ? `
                    <div class="video-upload-section">
                        <input type="file" id="video-upload-${playerId}" accept="video/mp4" style="display: none;">
                        <button class="btn-secondary" onclick="document.getElementById('video-upload-${playerId}').click()">Upload MP4 Video</button>
                    </div>
                ` : ''}
                <div class="highlights-grid" id="highlights-grid-${playerId}">
                    ...
                    `).join('') : '<p class="empty-state">No video highlights uploaded yet</p>'}
                </div>
            </div>
        `;
        
        if (this.isEditor) {
            const videoInput = document.getElementById(`video-upload-${playerId}`);
            if (videoInput) {
                videoInput.addEventListener('change', (e) => this.uploadHighlight(playerId, e));
            }
        }
    ...
    uploadHighlight(playerId, event) {
        ...
        reader.onload = (e) => {
            ...
            try {
                player.highlights.push(e.target.result);
                this.saveData('players', this.players);
                this.displayPlayerProfile(playerId);
                alert('Video uploaded successfully!');
            } catch (error) {
                if (error.name === 'QuotaExceededError') {
                    alert('Storage quota exceeded! Video files are very large. Consider uploading fewer or shorter videos.');
                } else {
    ...
    playVideo(playerId, videoIndex) {
        ...
        videoPlayer.src = player.highlights[videoIndex];
        videoPlayer.load();
        modal.classList.add('active');
        videoPlayer.play();
    }
    ...
    deleteHighlight(playerId, videoIndex) {
    ...
    selectRole(role) {
        ...
        if (role === 'admin' && !this.accessPasswords.admin) {
            this.openAdminSettings();
            return;
        }
        
        if (role === 'viewer' && !this.accessPasswords.viewer) {
    ...
    saveAccessPasswords() {
        ...
        const editorPass = document.getElementById('editor-password').value;
        const viewerPass = document.getElementById('viewer-password').value;
        ...
        alert('Access passwords saved successfully!');
