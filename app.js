...
function showMainApp() {
    ...
    setTimeout(() => {
        console.log('⏰ Delay complete, testing database access...');
        
        // Direct test - read players once
        database.ref('players').once('value')
            .then((snapshot) => {
                const data = snapshot.val();
                console.log('✅ PLAYERS READ SUCCESS:', data);
                console.log('✅ Player count:', data ? Object.keys(data).length : 0);
            })
            .catch((error) => {
                console.error('❌ PLAYERS READ FAILED:', error.code, error.message);
            });
