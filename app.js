class FootballDatabase {
    ...
    async rebuildPlayerDataFromGames() {
        if (!confirm('This will rebuild all player profiles from your game history. Any players without games will be removed. Continue?')) {
            return;
        }

        // Get all unique player IDs from games
        const playerIdsInGames = new Set();
        this.games.forEach(game => {
            game.playerStats.forEach(stat => {
                playerIdsInGames.add(stat.playerId);
            });
        });

        // Find or create players
        const rebuiltPlayers = [];
        playerIdsInGames.forEach(playerId => {
            let player = this.players.find(p => p.id === playerId);
            
            if (!player) {
                // Create a basic player profile if missing
                player = {
                    id: playerId,
                    firstName: 'Unknown',
                    lastName: 'Player',
            ...
            rebuiltPlayers.push(player);
        });

        this.players = rebuiltPlayers;
        ...
        alert(`Rebuilt ${this.players.length} player profile(s) from game history!`);
