class FootballDatabase {
    ...
    async loadAllData() {
        try {
            ...
            if (loadedPasswords) {
                this.accessPasswords = {
                    admin: loadedPasswords.admin || this.accessPasswords.admin,
                    editor: loadedPasswords.editor || this.accessPasswords.editor,
                    viewer: loadedPasswords.viewer || this.accessPasswords.viewer
                };
    ...
    playVideo(playerId, videoIndex) {
        ...
        if (videoType === 'youtube' || videoType === 'drive') {
            ...
            // Add autoplay and loop parameters to URL
            const separator = videoUrl.includes('?') ? '&' : '?';
            iframe.src = `${videoUrl}${separator}autoplay=1&loop=1&playlist=${videoUrl.split('/').pop()}`;
            iframe.width = '100%';
            iframe.height = '400';
